...
I'm trying to use your example to implements mitm proxy in my adblock app for 2 weeks, but still no luck. I'm newby in Java and sockets and this is a real hard task for me. If you have a time and opportunity, I'm asking for your help, not for free, of course. I've created repository on github with my android app, there are 2 packages that responsible for proxy solution proxy (TcpProxyServer) and tunnel.* I've tried to extend tunnel with your NioSslPeer class but there are a lot of differencies on methods wich I can't figure out.
 */
public abstract class NioSslPeer {

    private String Tag = "AdultBlock_NioSslPeer";
    /**
    ...
    protected boolean doHandshake(SocketChannel socketChannel, SSLEngine engine) throws IOException {
        ...
        Log.i(Tag, "About to do handshake...");
                ...
                case NEED_UNWRAP:
                    if (socketChannel.read(peerNetData) < 0) {
                        ...
                        try {
                            ...
                            Log.i(Tag, "This engine was forced to close inbound, without having received the proper SSL/TLS close notification message from the peer, due to end of stream.");
                        }
                    ...
                    try {
                        ...
                        Log.i(Tag, "A problem was encountered while processing the data that caused the SSLEngine to abort. Will try to properly close connection...");
                        engine.closeOutbound();
                        handshakeStatus = engine.getHandshakeStatus();
                        break;
                    }
                    ...
                    switch (result.getStatus()) {
                        ...
                        case CLOSED:
                            try {
                                ...
                                Log.i(Tag, "Failed to send server's CLOSE message due to socket channel's failure.");
                                handshakeStatus = engine.getHandshakeStatus();
                            }
                            break;
                        default:
